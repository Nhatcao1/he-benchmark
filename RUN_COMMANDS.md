...
`--kind workload`. Memory benchmarking is implemented with `--kind memory`:
...
./run_benchmarks.py --kind matrix --scheme ckks --all --ring-size 8192
...
OpenFHE six-thread. For OpenFHE-only scaling, add `--thread-scaling`; this
...
./run_benchmarks.py --thread-scaling --kind workload --scheme ckks --tests quick8 --ring-size 8192
```
...
started_unix=...,started_utc=...,test=quick8,threads=1,repeat=1,library=SEAL,scheme=CKKS,operation=add,size=8,ring_size=8192,correct=true,latency_ms=...,ops_per_sec=...,values_per_sec=...,mae=...,rmse=...,precision_bits=...
```
...
| Rotation-key generation | Generate Galois/rotation keys for +1, -1, +8 | BFV, BGV, CKKS | Implemented for BFV rotation and CKKS approximate | BFV: `--kind rotation`; CKKS: `--scheme ckks` |
...
| Rotate slots | Rotate packed vector +1, -1, +8 slots | BFV, BGV, CKKS | Implemented for BFV and CKKS | BFV: `--kind rotation`; CKKS: `--scheme ckks`; reports `rotate_1`, `rotate_-1`, `rotate_8` |
...
| CKKS matrix multiplication | 64 x 64 matrix multiplication | CKKS | Implemented as encrypted-row by plaintext-column CKKS workload | Run `./run_benchmarks.py --kind matrix --scheme ckks --all --ring-size 8192`; uses `he_corpus/matrices/matrix_*_0064x0064.csv` |
...
```bash
./run_benchmarks.py --scheme ckks --tests quick8 --ring-size 8192
grep -R "correct=false" cpp/results
```

Run broad CKKS approximate primitive coverage for all 8192-ring-compatible generated CKKS files:

```bash
./run_benchmarks.py --scheme ckks --all --ring-size 8192 --out-dir cpp/results/ckks_8192
grep -R "correct=false" cpp/results/ckks_8192
```

Run the 8192-row CKKS corpora with a larger ring:
...
./run_benchmarks.py --kind depth --scheme ckks --tests quick8 --ring-size 8192 --out-dir cpp/results/depth_ckks_quick
```
...
./run_benchmarks.py --kind workload --scheme ckks --all --ring-size 8192 --out-dir cpp/results/workload_ckks
```
...
./run_benchmarks.py --kind memory --scheme ckks --tests quick8 --ring-size 8192 --out-dir cpp/results/memory_ckks
```
...
./run_benchmarks.py --kind serialization --scheme ckks --tests quick8,normal256 --ring-size 8192 --out-dir cpp/results/serialization_ckks
```

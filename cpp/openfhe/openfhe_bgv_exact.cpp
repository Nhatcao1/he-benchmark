...
namespace
{
        ...
        std::size_t ring_size)
    {
        ...
        const auto components_before = encrypted_a->NumberCiphertextElements();

        auto mod_reduce_input = encrypted_a;
        const hebench::Timer timer;
        const auto switched = crypto_context->ModReduce(mod_reduce_input);
        const auto elapsed_ms = timer.elapsed_ms();

        const auto level_after = switched->GetLevel();
        const auto components_after = switched->NumberCiphertextElements();
        const auto decoded = decrypt_decode(crypto_context, private_key, switched, plain_modulus);
        ...
        print_metric_row(
            ...
            "level_before=" + std::to_string(level_before) +
                ...
                ",levels_dropped=" + std::to_string(level_after >= level_before ? level_after - level_before : 0) +
                ",components_before=" + std::to_string(components_before) +
                ",components_after=" + std::to_string(components_after) +
                ",supported=true" +
                (correct ? "" : ",error=\"" + error + "\""));
...
int main(int argc, char **argv)
{
    try
    {
        ...
        {
            throw std::runtime_error("corpus row count exceeds OpenFHE BGV batch size");
        }
    ...
    {
        std::cerr << "openfhe_bgv_exact failed: " << error.what() << '\n';
        return 2;
    }
}

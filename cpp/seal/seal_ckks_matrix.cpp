...
int main(int argc, char **argv)
{
    try
    {
        ...
        if (matrix_a.cols > slot_count)
        {
            throw std::runtime_error("matrix dimension exceeds CKKS slot count");
        }
        ...
        for (std::size_t row = 0; row < 64; ++row)
        {
            ...
            for (std::size_t col = 0; col < 64; ++col)
            {
                ...
                for (const auto step : rotation_steps(64))
                {
                    ...
                    evaluator.rotate_vector(cell, step, galois_keys, rotated);
        ...
        print_row(64, args.ring_size, max_abs_error <= 1e-2 || rmse <= 1e-3, elapsed_ms, max_abs_error, rmse, last_byte_size);
        return 0;
    }
    catch (const std::exception &error)
    {
        std::cerr << "seal_ckks_matrix failed: " << error.what() << '\n';
        return 2;
    }
}

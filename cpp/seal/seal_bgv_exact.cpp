...
int main(int argc, char **argv)
{
    try
    {
        ...
        // BGV batching capacity is fixed by the parameter set. Larger corpus
        // files need a bigger poly modulus degree or a chunking runner.
        if (rows.size() > slot_count)
        {
            throw std::runtime_error("corpus row count exceeds SEAL BGV slot count");
        }
    ...
    {
        std::cerr << "seal_bgv_exact failed: " << error.what() << '\n';
        return 2;
    }
}
